...
class InferenceRecipe:
    ...
    def generate(self, cfg: DictConfig):
        ...
        # I changed it to return the generated text
        generated_text = self._tokenizer.decode(generated_tokens[0])
        ...
        # Allows the model to return the generated text instead of just output it in Logger
        return generated_text
    
#to be able to use multiple inputs 
#@config.parse
def main(cfg: DictConfig) -> None:
    config.log_config(recipe_name="InferenceRecipe", cfg=cfg)
    recipe = InferenceRecipe(cfg=cfg)
    recipe.setup(cfg=cfg)
    result = recipe.generate(cfg=cfg)
